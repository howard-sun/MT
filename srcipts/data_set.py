...
class SplittedDataset(object):
    def __init__(self,input_dir,lowercase=False,lang_src='de',lang_tgt='en',path_format='{split}.bpe.{lang}',source_attach_ends=True):
        self.source_attach_ends = source_attach_ends
        trn_src_path = os.path.join(input_dir, path_format.format(split='train', lang=lang_src))
        trn_tgt_path = os.path.join(input_dir, path_format.format(split='train', lang=lang_tgt))

        val_src_path = os.path.join(input_dir, path_format.format(split='valid', lang=lang_src))
        val_tgt_path = os.path.join(input_dir, path_format.format(split='valid', lang=lang_tgt))

        tst_src_path = os.path.join(input_dir, path_format.format(split='test', lang=lang_src))
        tst_tgt_path = os.path.join(input_dir, path_format.format(split='test', lang=lang_tgt))

        # 加载vocab
        vocab_src_path = os.path.join(input_dir, path_format.format(split='vocab', lang=lang_src))
        vocab_tgt_path = os.path.join(input_dir, path_format.format(split='vocab', lang=lang_tgt))

        self.vocab_src = Vocab.load(vocab_src_path,train_path=trn_src_path,lowercase=lowercase)
        self.vocab_tgt = Vocab.load(vocab_tgt_path,train_path=trn_tgt_path,lowercase=lowercase)

        # 对数据集进行操作
        self.data_sets = {
            'trn':
            SingleDataset(trn_src_path,trn_tgt_path,vocab_src=self.vocab_src, vocab_tgt=self.vocab_tgt,source_attach_ends=source_attach_ends),
            'val':
            SingleDataset(val_src_path,val_tgt_path,vocab_src=self.vocab_src,vocab_tgt=self.vocab_tgt,source_attach_ends=source_attach_ends),
            'tst':
            SingleDataset(tst_src_path,tst_tgt_path, vocab_src=self.vocab_src,vocab_tgt=self.vocab_tgt,source_attach_ends=source_attach_ends),
        }
...
class SingleDataset(Dataset):
    def __init__(self,path_src,path_tgt,vocab_src,vocab_tgt,source_attach_ends=True):
...
class LenMatchBatchSampler(torch.utils.data.BatchSampler):
    ...
    def _exploded(self, batch_len, next_len):
        if batch_len == 0:
            return False
        if (self.max_tokens is not None and self.max_tokens > 0 and batch_len + next_len > self.max_tokens):
            return True
        return False
    ...
    def __iter__(self):
        ...
        assert len(self) == yielded, 'produced an incorrect number of batches. %d != %d' % (
            len(self), yielded)
        yielded_idx = set()
        for indices in yielded_indices:
            yielded_idx.update(indices)
        assert len(self.sampler) == len(yielded_idx), 'produced an incorrect number of instances. %d != %d' % (len(
            self.sampler), len(yielded_idx))


def get_dataloader(dset,batch_size,max_tokens=None,shuffle=True,sample_by_length=True,num_workers=2, pin_memory=True):
