...
class Trainer(object):
    def __init__(self, args, data_splits, device):
        ...
        self.model = models.transformer.Transformer(args=args,src_dict=data_splits.vocab_src,tgt_dict=data_splits.vocab_tgt,).to(device)

        if args.optim == 'adam':
            self.optimizer = torch.optim.Adam(params=self.model.parameters(),lr=args.learning_rate,betas=args.betas,weight_decay=args.weight_decay)
        ...
        if args.decay_method == 'inverse_sqrt' and not args.fp16:
            self.scheduler = optim.lr_scheduler.InverseSqrtScheduler(self.optimizer,warmup_steps=args.warmup_steps,min_lr=args.min_lr)

        self.train_loader = data_set.get_dataloader(dset=data_splits['trn'],batch_size=args.batch_size,max_tokens=args.max_tokens,pin_memory=not self.cpu_only)
        self.val_loader = data_set.get_dataloader( dset=data_splits['val'],batch_size=args.eval_batch_size,pin_memory=not self.cpu_only)
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.max_epochs + 1):
            ...
            for idx, batch in enumerate(
                ...
                loss, nll = losses.masked_nll(logits=logits,lengths=tgt_lens,targets=tgt_out,label_smoothing=self.args.label_smoothing)
                ...
                print(('\r[step {:4d}/{}] loss: {:.3f}, '
                       'nll loss: {:.3f}, ppl: {:.3f}, time: {:.1f}s').format(idx + 1,len(self.train_loader),avg_loss,avg_nll,avg_ppl,cur_time - begin_time),end='')
...
def main():
    ...
    if args.mode == 'train':
        ...
        for path in args.init_checkpoint:
            ...
            print('{}\t{}\t{}\t{}\t{}'.format(path, val_nll, val_ppl, tst_nll,tst_ppl))
